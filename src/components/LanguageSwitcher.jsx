...
const LanguageSwitcher = () => {
  ...
  return (
    <div className="flex items-center justify-center space-x-1 sm:space-x-2 rounded-full bg-gray-200 dark:bg-gray-700 p-0.5 sm:p-1">
      {Object.keys(languages).map((lng, index) => (
        <React.Fragment key={lng}>
          <button
            onClick={() => setLang(lng)}
            className={`
              px-2 sm:px-3 py-0.5 sm:py-1 text-xs sm:text-sm font-semibold rounded-full transition-all duration-300
              ${
                lang === lng
                  ? "bg-blue-500 text-white scale-105 sm:scale-110 shadow-md"

  ...
  const testimonials = [
    {
      ...
      project: {
        es: 'Pintura interior - Piso en Valencia',
        ca: 'Pintura interior - Pis a València',
        en: 'Interior Painting - Apartment in Valencia'
      },
      text: {
    ...
    },
    {
      ...
      project: {
        es: 'Renovación completa - Casa familiar en Madrid',
        ca: 'Renovació completa - Casa familiar a Madrid',
        en: 'Full Renovation - Family Home in Madrid'
      },
      text: {
    ...
    <section id="testimonials" className="py-24 bg-gray-50 dark:bg-gray-800/50">
      <div className="container mx-auto px-6" data-aos="fade-up">
        ...
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-primary-light dark:text-white">
            {t.testimonials?.title || 'Nuestros Clientes Hablan'}
          </h2>
          <p className="mt-4 text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            {t.testimonials?.subtitle || 'Opiniones reales de personas que confiaron en nosotros'}
          </p>
        </div>
            ...
            >
              {testimonials.map((testimonial, index) => (
                <div key={index} className="w-full flex-shrink-0 px-4">
                  <div className="bg-white dark:bg-gray-700 p-10 rounded-2xl shadow-lg text-center">
                    <img
                      src={`https://i.pravatar.cc/150?img=${index + 1}`}
                      alt={testimonial.name[lang] || 'Cliente'}
                      className="w-24 h-24 rounded-full mx-auto mb-6 border-4 border-secondary-light"
                    />
                    ...
                    <p className="text-lg text-gray-700 dark:text-gray-200 italic mb-6 leading-relaxed">
                      "{testimonial.text[lang] || testimonial.text.es}"
                    </p>

                    <h4 className="font-bold text-xl font-display text-primary-light dark:text-white">
                      {testimonial.name[lang] || testimonial.name.es}
                    </h4>
                    <p className="text-secondary-light dark:text-yellow-400 font-medium">
            ...
            {testimonials.map((_, idx) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all ${
                  idx === currentIndex ? 'bg-secondary scale-125' : 'bg-gray-300 dark:bg-gray-600 hover:bg-gray-400'
                }`}
